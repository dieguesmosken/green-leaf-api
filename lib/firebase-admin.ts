...
export function initializeFirebaseAdmin() {
  if (getApps().length === 0) {    // Using the correct project configuration
    const projectConfig = {
      projectId: "greenleafaxis",
      storageBucket: "greenleafaxis.firebasestorage.app",
    }

    if (process.env.FIREBASE_ADMIN_PRIVATE_KEY) {      const serviceAccount = {
        projectId: "greenleafaxis",
...
export const adminDb = getFirestore()  // Usando banco padrão que funciona
